...
typedef R<Z,S1<N,U<3,3>>>gv;
//typedef getVal;

typedef S1<Z, U<2,1>> Z2;
//typedef R<Z2, S3<	,U<4,4>, 	S1<N,U<4,4>> > tmp;
//typedef R<Z2, S3<   S2<sum, U<4,1>, S2<mul, U<4,4>,U<4,2> >  > ,U<4,4>, 	S1<N,U<4,4>>> > tmp;
//typedef R<Z2, S3<iff , S2<sub , U<4,1> , S2<mul, U<4,4> , U<4,2>>  >   ,U<4,4>, 	S1<N,U<4,4>>    >  > tmp;
typedef R<Z2, S3< iff ,S2<sub, U<4,1>, S2<mul,U<4,2>, U<4,4> > >   ,S1<N,U<4,4>>,  U<4,4> >  > tmp;

typedef S3<tmp, divmax,U<2,2>,U<2,1> >dvd;
//typedef S1<sub1, dd>dvd;
...
int main()
{
	std::vector <unsigned> xy {19, 2};
    std::cout << "f(" << xy[0]<< ", " <<xy[1]  << ") = " << dvd::compute(xy) << "\n";
    return 0;
