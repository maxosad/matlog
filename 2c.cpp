...
//typedef R<Z,S3<S2<mod, U<3,1>, S1<N,S1<N,S1<Z,U<3,1>>>>>, U<3,3>, S1<N,U<3,3>>  >> tt;

typedef S1<N,S1<N,S1<Z,U<3,1>>>> two;
typedef S1<N,S1<N,S1<Z,U<1,1>>>> two1;
typedef S1<N,Z> uno;
typedef R<uno,S2 <mul, U<3,1>, U<3,3>>  > st;

typedef R<Z, S3<iff, S2<mod, U<3,1>, S2<st,two,U<3,3>>>   ,U<3,3> ,S1<N,U<3,3>>>  > tt;
typedef S2<tt,U<1,1>,U<1,1>> hh;
typedef S1<sub1,hh> head;
...
int main()
{
	std::vector <unsigned> xy {20 };
    std::cout << "f(" << xy[0]  << ") = " << head::compute(xy) << "\n";
    return 0;
